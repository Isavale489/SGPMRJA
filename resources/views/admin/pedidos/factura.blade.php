...
<head>
    ...
    <style>
        ...
        .company-info { text-align: center; font-size: 11px; }
        .info-table, .items-table, .totals-table { width: 100%; border-collapse: collapse; margin-top: 8px; }
        /* Forzar mismas dimensiones */
        .info-table, .items-table { table-layout: fixed; }
        /* Definir proporciones de columnas en items-table */
        .items-table th:nth-child(1), .items-table td:nth-child(1) { width: 7%; }
        .items-table th:nth-child(2), .items-table td:nth-child(2) { width: 35%; } /* Concepto o Descripción del Producto */
        .items-table th:nth-child(3), .items-table td:nth-child(3) { width: 10%; } /* Modelo */
        .items-table th:nth-child(4), .items-table td:nth-child(4) { width: 7%; } /* Color */
        .items-table th:nth-child(5), .items-table td:nth-child(5) { width: 7%; } /* Talla */
        .items-table th:nth-child(6), .items-table td:nth-child(6) { width: 15%; } /* Logo */
        .items-table th:nth-child(7), .items-table td:nth-child(7) { width: 9%; } /* Costo Unitario */
        .items-table th:nth-child(8), .items-table td:nth-child(8) { width: 10%; } /* Monto */
        .info-table td {
            border: 1px solid #000;
            padding: 4px;
            font-size: 11px;
            word-break: break-all; /* evita desbordes largos como emails */
        }
        .items-table th, .items-table td { border: 1px solid #000; padding: 4px; text-align: center; font-size: 11px; }
        .items-table th { background-color: #eaeaea; }
        .totals-table { margin-left: auto; } /* Alinea totales a la derecha dentro del contenedor */
        .totals-table td { padding: 4px; font-size: 11px; }
        .totals-table .label { text-align: right; }
        .totals-table .value { text-align: right; width: 80px; }
        .note { background-color: #ffff00; margin-top: 12px; padding: 8px; font-size: 11px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin: 10px 0 5px 0;">
            <div style="flex: 1; text-align: center; font-size: 15px; font-weight: bold;">N° de Pedido: {{ $pedido->id }}</div>
            <div style="flex: 1; text-align: right; font-size: 13px; font-weight: bold;">Elaborado por: {{ $pedido->user->name ?? '-' }}</div>
        </div>
        ...
        <table class="info-table">
            <tr>
                ...
                <td style="width: 220px;">{{ $pedido->cliente_nombre }}</td>
                <td style="width: 60px; font-weight:bold;">
                    @if(str_starts_with($pedido->ci_rif, 'V-'))
                        C.I.:
                    @else
                        Rif:
                    @endif
                ...
                <td>{{ $pedido->fecha_entrega_estimada ? \Carbon\Carbon::parse($pedido->fecha_entrega_estimada)->format('d/m/Y') : '-' }}</td>
            </tr>
            <tr>
                <td style="font-weight:bold;">Teléfono Contacto:</td>
                <td>{{ $pedido->cliente_telefono }}</td>
                <td style="font-weight:bold;">e-mail:</td>
                <td colspan="5" style="word-break: break-all; word-wrap: break-word; max-width: 100%;">{{ $pedido->cliente_email }}</td>
            </tr>
        </table>
