    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-light p-3">
                <h5 class="modal-title" id="createModalLabel">Registrar Movimiento de Existencia</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form id="createForm">
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="insumo_id" class="form-label required">Insumo</label><select class="form-select" id="insumo_id" name="insumo_id" required>
                            <option value="">Seleccione un insumo</option>
                            @foreach($insumos as $insumo)
                                <option value="{{ $insumo->id }}" data-stock="{{ $insumo->stock_actual }}" data-unidad="{{ $insumo->unidad_medida }}">
                                    {{ $insumo->nombre }} ({{ $insumo->tipo }}) - Stock: {{ $insumo->stock_actual }} {{ $insumo->unidad_medida }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        <label for="tipo_movimiento" class="form-label required">Tipo de Movimiento</label><select class="form-select" id="tipo_movimiento" name="tipo_movimiento" required>
